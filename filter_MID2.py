...
for i in range(len(tlxList)):
  ...
  df_genome=pd.read_csv(fileName[0],sep='\t')
  ...
  stats=stats.append(a,ignore_index=True)

stats_file= stats.to_csv(os.path.join(args.output_folder, 'stats.csv'),index=False)
